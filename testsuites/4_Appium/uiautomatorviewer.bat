...
for /f %%a in ("%APP_HOME%\lib\uiautomatorviewer-25.3.2.jar") do set jarfile=%%~nxa
set frameworkdir=.
...
echo "======================= UIAutomator ViewerRunning ======================="

rem call "%java_exe%" "-Djava.ext.dirs=%javaextdirs%" "-Dcom.android.uiautomator.bindir=%prog_dir%" -jar %jarpath% %*
call "%java_exe%" "-Djava.ext.dirs=%javaextdirs%" "-Dcom.android.uiautomator.bindir=C:\AndroidSDK\tools" -jar %jarpath% %*